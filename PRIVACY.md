# Privacy Policy

Last updated: February 16, 2026

`al-Iyaal Kids` is a local-first desktop app.

## What we collect

- We do not collect personal information.
- We do not collect analytics or telemetry in the MVP. The analytics tab is local specifically for your viewing.
- We do not upload your media files to our servers.

## How data is processed

- Video/audio processing happens on your device.
- Temporary/runtime files may be created locally to run Demucs, Python runtime assets, and ffmpeg.

## Third-party services

- No third-party tracking or analytics SDKs are included in the MVP.
- If future versions add optional cloud features, this policy will be updated before release.

## Contact

- GitHub: https://github.com/ragaeeb/al-iyaal-kids
